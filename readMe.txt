...
각 데이터 셋은 orign data와 같은 크기의 1000개의 csv파일로 구성되고 각 csv 파일은 시계열 데이터 값(value)과 데이터 값이 오류 label(error) 오류값의 크기(error_size)로 구성됩니다.
오류 label은 오류가 없으면 0,drift면 1, hard-over 면 2, erratic이면 3, spike면 4, stuck면 5입니다.
